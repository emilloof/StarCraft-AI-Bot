...
class SafeMove(Task):
    ...
    def on_step(self, py_unit: PyUnit) -> Status:
        ...
        if py_unit.is_alive:
            ...
            if(self.agent.map_tools.get_ground_distance(py_unit.position, self.target) == 0):
                return Status.DONE
            else:
    ...
    def update_path(self, py_unit: PyUnit):
        ...
        if self.agent.vertex_dict.get((start_position[0], start_position[1])) is None:
            self.agent.vertex_dict[(start_position[0], start_position[1])] = Vertex((start_position[0], start_position[1]))    
